...
export const Dashboard = () => {
  ...
  const columns = [
    ...
    },
    {
      key: 'status',
      title: 'Status',
      render: (_, row) => <Badge variant={row.status_color}>{row.status_label}</Badge>,
    },
  ];
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard title="Penjualan Hari Ini" value={formatRupiah(stats?.data?.today_sales || 0)} icon={TrendingUp} />
        <StatCard title="Total Pesanan" value={stats?.data?.total_orders || 0} icon={ShoppingBag} />
        <StatCard title="Produk Terjual" value={stats?.data?.products_sold || 0} icon={Package} />
        <StatCard
          title="Stok Menipis"
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <DataTable columns={columns} data={stats?.data?.recent_orders || []} loading={loading} emptyMessage="Belum ada pesanan" />
      </div>
    </section>
  );
};
